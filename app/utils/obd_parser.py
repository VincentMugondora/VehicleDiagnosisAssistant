...
def _normalize_code(letter: str, digits: str) -> str:
    """Handle OCR errors: replace letter 'O' with digit '0'"""
...
def parse_message(text: str) -> Dict[str, Optional[str]]:
    ...
    Args:
        text: Raw message text (e.g., "P0171 Toyota Corolla 2015 1.6L")

    Returns:
        Dict with keys: code, make, model, year, engine
        All values are Optional[str] - will be None if not found
    """
    text = (text or "").strip()
